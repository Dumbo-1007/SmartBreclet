...
//此次波峰的时间  
long timOfThisPeak = 0;  
//上次波峰的时间  
long timOfLastPeak = 0;  
//当前的时间  
long timOfNow = 0;  
//当前传感器的值  
float gravityNew = 0;  
...
* 修改记录：
*******************************************************************************/
sportsInfo_t *onSensorChanged(accValue_t *pAccValue,timStamp_t *timStamp_p,personInfo_t * personInfo) {  
  accValue_t *p = pAccValue;
  personInfo_t *userInfo = personInfo;
  timStamp_t *tim_p = timStamp_p;
  oriValues[0] = p->accX;
  oriValues[1] = p->accY;
  oriValues[2] = p->accZ;
  //对三轴数据进行平方和开根号的处理 
  ...
  //检测步子
  return DetectorNewStep(gravityNew,tim_p,userInfo); 
}  
/*******************************************************************************
* 函数名：DetectorNewStep
* 功能描述： 
...
* 修改记录：
*******************************************************************************/
sportsInfo_t *DetectorNewStep(float values,timStamp_t *timStamp_p,personInfo_t * personInfo) 
{  
  static u32 tim_old;
  personInfo_t *userInfo = personInfo;
  ...
  {  
    ...
    {  
      timOfLastPeak = timOfThisPeak;//更新上次波峰的时间  
      //将时间戳转换为以毫秒ms为单位
      tim_now = timOfNow = ((tim_p->hour*60+tim_p->minute)*60+tim_p->second)*1000+tim_p->twentyMsCount*20; //获取时间 ,并转化为毫秒
      //如果检测到了波峰，并且符合时间差以及阈值的条件，则判定为1步 
      if (  (timOfNow - timOfLastPeak >= 250 )//TAO 修改为300，防止轻微动都也会检测步子
          //&& (timOfNow - timOfLastPeak <= 2000)
          &&(peakOfWave - valleyOfWave >= ThreadValue)
            )
      {  
        timOfThisPeak = timOfNow; //更新此次波峰时间 
        ...
        if((tim_now - tim_old) >= 2000 )    //如果时间过了2秒
        {
        ...
        * */  				
        if ((stepTempCount< 5 )&&(timOfNow - timOfLastPeak >= 3000))          
        {
          stepTempCount = 0;
        }
        else if((stepTempCount>= 5)&&(timOfNow - timOfLastPeak <= 3000))
        {
      ...
      if (timOfNow - timOfLastPeak >= 250  
          && (peakOfWave - valleyOfWave >= initialValue)) 
      {  
        timOfThisPeak = timOfNow;  
